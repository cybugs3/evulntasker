...
def evulntasker_icon() -> FileResponse:
    ...
    for path in (static_icon, root_icon, BASE_DIR.parent.parent / "VAICT-ICON.png"):
        if path.is_file():
...
def repositories_page(request: Request) -> HTMLResponse:
    return render_page(request, "repositories.html", active="repositories")
...
def tracker_page(request: Request) -> HTMLResponse:
    return render_page(request, "tracker.html", active="tracker")
